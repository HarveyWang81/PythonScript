...
def op1(event):
    finename.write("1")  # 调用函数时,将 括号内 数字 写入文本当中
...
s1_box, s2_box, s3_box, s4_box, = wx.BoxSizer(wx.VERTICAL), wx.BoxSizer(wx.VERTICAL), wx.BoxSizer(
    wx.VERTICAL), wx.BoxSizer(wx.VERTICAL)
